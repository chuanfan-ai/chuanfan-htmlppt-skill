...
用于把用户提供的产品截图、网页截图、代码截图、设计稿截图处理成符合模板比例的图片资产。目标是类似 CleanShot X 的“截图居中 + 背景填充 + 统一比例”,默认保留原截图,不让图片模型重画真实界面。

## 优先级

1. **程序化适配优先**:截图内容、文字、UI 细节需要保真时,不要重画;创建目标比例画布,把原截图等比缩放后放入画布。
2. **图片模型只做获准的示意重构**:只有原图过长、过窄、信息太乱、需要 UI 情景化或概念化表达,并且用户明确允许时,才使用当前已配置的图片模型生成“截图再设计 / UI 情景图”。生成结果必须标注为示意,不能冒充真实产品截图。
...
2. **再选处理方式**:
   ...
   - 原图不可用或需要解释概念:在用户允许时,再使用当前已配置的图片模型生成示意图。
...
- 背景: `plain` / `grid` / 白色到 `#eaf1ff` 的低对比平面过渡
- 色彩:白、黑、品牌蓝为主,品牌橙只做小面积行动提示
- 截图:小圆角或直角、轻微边线、默认无重阴影,保证投屏可读
- 背景优先由 CSS 生成,不需要为每张截图调用图片模型
- 推荐语义:

```text
ratio:16:10, background:plain, padding:standard, inset:subtle, shadow:none, corners:small, alignment:center
```
...
- 风格 C 默认用 CSS 的纯色、细网格或低对比蓝色平面生成背景,不依赖额外位图资产。

## 内置主题背景资产

本 Skill 已经内置一组预生成背景。处理截图时**优先使用这些资产**,不要实时调用图片模型重新生成背景。只有用户明确要求新风格、现有主题缺失,或背景与内容明显不匹配时,才使用当前已配置且获准的图片模型生成新背景。
...
| 概念解释用的 UI 情景图 | 用户允许时可用当前已配置图片模型重新设计,并标注为示意 |
