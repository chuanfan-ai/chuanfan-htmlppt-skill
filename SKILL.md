...
- **演示现场编辑**:模板自带本地编辑配置面板。文字编辑和新增浮层存 `localStorage`;图片替换/新增必须优先通过 `scripts/local-edit-server.py` 落盘到 `images/user-edits/`,HTML 只保存相对路径,不要把大图片 base64 写进 `localStorage`
...
**编辑模式规则**:
- 文字改动可以自动保存到 `localStorage`,适合现场改错字
- 图片替换和新增图片必须走本地编辑服务,保存为真实文件路径
- 可以在当前页添加文字、添加图片、拖动位置、拖右下角控制点缩放、删除选中元素
- 新增内容是每页上方的浮层,不会改写原始 slide HTML 结构,也不会影响原有布局和动效
- `file://` 直接打开仍可演示、全屏、查看配置;要落盘保存图片,用上面的服务 URL 打开
- 交付给别人时,带上整个 PPT 文件夹,包括 `images/user-edits/`
