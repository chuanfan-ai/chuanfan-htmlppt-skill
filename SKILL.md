...
- 大段表格数据、图表叠加（用常规 PPT）
- 培训课件（信息密度不够）
- 需要多人协作编辑或完整 PPT 结构编辑（这是静态 HTML；只提供本地演示级编辑浮层）
...
- **演示现场编辑**:模板自带本地编辑配置面板。文字编辑和新增浮层存 `localStorage`;图片替换/新增必须优先通过 `scripts/local-edit-server.py` 落盘到 `images/user-edits/`,HTML 只保存相对路径,不要把大图片 base64 写进 `localStorage`。编辑器不能退化:必须保留 `配置 / 编辑 / 添加文字 / 添加图片 / 删除选中 / 全屏 / 保存版本 / 历史版本 / 重置` 完整工具栏,不要另写一套简化编辑器覆盖模板能力
...
#### 3.0.6 · 观众视角文案转换（素材保留但表达重写）

当用户给的是飞书文档、文章、旧 PPT 或素材包时，必须区分“保留内容”和“照搬原文结构”。图片、视频、提示词可以尽量完整保留，但页面上的讲述语言必须转成演讲现场能直接给观众看的表达。

**硬规则:**
- PPT 给观众看的文字，不能写成制作说明。像“这页只放一个提示词”“适合投屏复制”“原文素材保留”“控制条可直接播放”这类话，对观众没有价值，必须删除或改写。
- 每页都要问一句：这句话是在帮听众建立认知、制造记忆点、推进观点，还是只是给制作者看的备注？如果是后者，删掉或改成观众能听懂的金句 / 方法论 / 传播话术。
- 提示词页左侧不解释页面怎么做；左侧放 takeaway、金句、使用方法或传播话术，右侧完整保留提示词正文。提示词正文不能为了美观而缺段、缺参数、缺约束。
- 视频素材如果没有强关联，就一页一个视频。不要把多个无关联视频合在一页；单页标题要说明“看什么”和“为什么看”，而不是写“单视频页”。
- “内容尽量全部保留”不等于“原文结构和原文说明照搬”。素材、提示词、视频要保留，呈现语言要转成演讲逻辑。

**常见替换方式:**
- “原文开场素材” → “AI 不是只属于技术团队，它正在进入每个人的工作桌面。”
- “这页只放一个提示词，字号放大” → “把细节说清楚，AI 才不会用‘差不多’糊弄你。”
- “单视频页 · 控制条可直接播放” → “点击播放，看 AI 如何进入具体业务现场。”
- “原文提示词单独放大” → “先定规则，再要答案。”

交付前必须全文搜索这些制作说明词：`原文`、`保留`、`这页`、`单视频页`、`字号放大`、`适合投屏`、`复制`、`控制条`、`文档开头`、`后面`。如果命中的是观众可见文案，必须改写；如果只在 CSS 注释或真实提示词正文里，才可以保留。
...
普通播放不需要本地服务器。图片走相对路径 `images/xxx.png`。

如果用户需要在浏览器里**持久化替换图片**,启动本地编辑服务:

```bash
python3 "<SKILL_ROOT>/scripts/local-edit-server.py" --deck-dir "项目/XXX/ppt" --open
```

本地编辑服务只写当前 deck 目录下的 `images/user-edits/`,并按图片内容 hash 去重。同一张图重复替换会复用旧文件,不会复制多份。打开 PPT 时模板会默认弹出"本地编辑配置"面板,显示服务地址、启动命令和连接状态;服务未连接时,图片替换不允许持久化为 base64,避免撑爆浏览器存储。

**编辑模式规则**:
- 文字改动自动保存到 `localStorage`,适合现场改错字;提示词、代码块、列表、卡片正文也必须进入可编辑选择器,不要只覆盖 `h/p/li`
- 图片替换和新增图片必须走本地编辑服务,保存为真实文件路径;如默认端口异常,先用 `curl --noproxy '*'` 检测本地服务,不要被系统代理误导
- 可以在当前页添加文字、添加图片、拖动位置、拖右下角控制点缩放、删除选中元素
- 新增内容是每页上方的浮层,不会改写原始 slide HTML 结构,也不会影响原有布局和动效;透明编辑层 `.g-edit-layer` 必须保持 `pointer-events:none`,只让 `.g-overlay-item` 接收鼠标事件,否则会挡住底层文字和图片导致"点了编辑但不能编辑"
- `保存版本` 必须保存当前文字、图片替换、新增文字/图片浮层状态;`历史版本` 必须允许恢复任意保存点,恢复前自动创建一份"恢复前自动备份"
- `file://` 直接打开仍可演示、全屏、查看配置、编辑文字和使用历史版本;要落盘保存图片,用上面的服务 URL 打开
- 交付给别人时,带上整个 PPT 文件夹,包括 `images/user-edits/`;历史版本存在浏览器 `localStorage`,和页面路径绑定

**生成/迭代安全规则**:
- 修改已有 HTMLPPT 前,先复制完整 deck 目录备份,不要直接覆盖原 `index.html`、`slides-fragment.html`、`images/` 和素材 manifest
- 如果用户说"基于旧版修改",新版必须放到新目录或先备份旧目录;不要把新旧版本写到同一个 `outputs/xxx/index.html`
- 不要把文件级校验当成功能验收。交付前至少检查:工具栏按钮存在、点击编辑后目标文字出现 `contenteditable=true`、图片点击触发替换、`localStorage` 写入编辑状态、`保存版本/历史版本` 能出现保存点、素材无缺失、版式校验通过
...
│   └── local-edit-server.py    ← 可选本地编辑服务:保存替换图片到 images/user-edits/ 并 hash 去重
└── references/
